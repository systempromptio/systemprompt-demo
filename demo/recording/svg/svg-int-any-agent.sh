...
curl -s -X POST "$BASE_URL/api/public/hooks/govern?plugin_id=svg-demo" \
  ...
  -d '{"hook_event_name":"PreToolUse","tool_name":"mcp__admin__list_agents","agent_id":"developer_agent","session_id":"svg-any-agent","cwd":"/var/www/html/systemprompt-template","tool_input":{}}' 2>/dev/null \
  | color_json
echo ""
...
curl -s -X POST "$BASE_URL/api/public/hooks/govern?plugin_id=svg-demo" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"hook_event_name":"PreToolUse","tool_name":"mcp__admin__list_agents","agent_id":"associate_agent","session_id":"svg-any-agent","cwd":"/var/www/html/systemprompt-template","tool_input":{}}' 2>/dev/null \
  | color_json
echo ""
