...
//!
//! Core's engine is caller-owned and stateless about location; this module
//! supplies the two deployment decisions core leaves open — where the config
//! lives (`<services>/governance/config.yaml` per the profile) and that both
//! enforcement points (the `/hooks/govern` webhook and the pi in-process seam)
//! share one engine, so the rate limiter sees every call.
