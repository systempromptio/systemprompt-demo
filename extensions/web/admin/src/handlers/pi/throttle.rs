...
//!
//! The client IP comes from core's trust-gated resolver, which believes a
//! forwarding header only when the peer is listed in `server.trusted_proxies`
//! and otherwise keys on the socket peer. Reading the header directly would
//! make every bucket client-selectable, and a limiter an attacker can rekey at
//! will is not one.
...
use systemprompt::api::services::middleware::client_addr::client_ip_from_request;
...
pub(super) async fn per_ip(State(lane): State<Arc<Lane>>, req: Request, next: Next) -> Response {
    // Why: an unresolved address is one the served router always supplies, so
    // this is an in-process caller — fail open rather than throttle everyone
    // into one bucket, the same trade `Lane::allow` makes on a poisoned mutex.
    let Some(ip) = client_ip_from_request(&req) else {
        return next.run(req).await;
    };

    if lane.limit == 0 || lane.allow(&ip.to_string()) {
        return next.run(req).await;
    }
    (
