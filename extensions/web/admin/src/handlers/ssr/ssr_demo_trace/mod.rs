...
/// The session-detail page is mounted under `/admin/entities/`, and a
/// hand-written `/admin/sessions/{id}` link 404s. Spelled out once here.
fn session_detail_url(session: &SessionId) -> String {
    format!(
...
pub(crate) async fn demo_trace_page(
    ...
    let ctx = DemoTraceContext {
        ...
        session_detail_url: session_detail_url(&session_id),
        session_id,
