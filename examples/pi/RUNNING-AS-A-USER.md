...
scripts pick it up automatically. The examples below assume 8080; substitute
your own.
...
```
http://localhost:8080/admin/user?id=<id>            profile, events, costs
http://localhost:8080/admin/models?user_id=<id>          per-user model access
http://localhost:8080/admin/demo/trace?session=<session> the governed timeline
```
...
| Connection refused from a script | `localhost` resolving to `[::1]` | `BASE_URL=http://127.0.0.1:8080 <script>` |
