...
# Shared with the quality workflow via scripts/check-no-synthesis.sh so the
# local and CI gates cannot drift; the rule and its exemption marker are
# documented in the script header.
lint-no-synthesis:
    bash scripts/check-no-synthesis.sh
...
prepare:
    ...
    EXTENSION_DIRS="extensions/cli/activity extensions/cli/slack extensions/credits extensions/web extensions/marketplace extensions/mcp/shared extensions/mcp/systemprompt"
...
setup-local ANTHROPIC_KEY="" OPENAI_KEY="" GEMINI_KEY="" HTTP_PORT="8080" PG_PORT="5432":
    bash scripts/setup-local.sh "{{ANTHROPIC_KEY}}" "{{OPENAI_KEY}}" "{{GEMINI_KEY}}" "{{HTTP_PORT}}" "{{PG_PORT}}"
tenants:
